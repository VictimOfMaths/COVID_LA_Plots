...
temp <- tempfile()
source <- "https://www.opendata.nhs.scot/dataset/b318bddf-a4dc-4262-971f-0ba329e09b87/resource/427f9a25-db22-4014-a3bc-893b68243055/download/trend_ca_20201101.csv"
temp <- curl_download(url=source, destfile=temp, quiet=FALSE, mode="wb")
...
temp <- tempfile()
source <- "https://www.health-ni.gov.uk/sites/default/files/publications/health/doh-dd-011120.xlsx"
temp <- curl_download(url=source, destfile=temp, quiet=FALSE, mode="wb")
#Need to update the range here too:
casedata.NI <- read_excel(temp, sheet=3, range="A2:E3176", col_names=FALSE)
