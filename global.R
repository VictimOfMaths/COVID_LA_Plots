...
#Shiny kept throwing an 'Out of memory' error with the below code so have moved the
#data processing step offline and will have to update this regularly until I work
#out how to avoid this

#Massive thanks to Adam Watkins (@agcwatkins) for helping speed this code up
...
#  data <- read_csv(temp, col_types="cccDciid") %>% 
#    select(c(1:6)) %>% 
#    filter(areaType=="ltla" & age!="unassigned") %>% 
