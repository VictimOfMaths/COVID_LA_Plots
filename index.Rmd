...
spark_plots1 <- daydata %>% 
  as.data.frame() %>% 
  filter(name %in% unique(sparknamedata1$name) & date>maxdate-months(1)) %>% 
...
spark_plots2 <- daydata %>% 
  as.data.frame() %>% 
  filter(name %in% unique(sparknamedata2$name) & date>maxdate-months(1)) %>% 
...
spark_plots3 <- daydata %>% 
  ...
  mutate(cases_change=casesroll_avg-lag(casesroll_avg, 7)) %>% 
  ungroup() %>% 
  filter(name %in% unique(sparknamedata3$name) & date>maxdate-months(1)) %>% 
...
spark_plots4 <- daydata %>% 
  ...
  ungroup() %>% 
  filter(name %in% unique(sparknamedata4$name) & date>maxdate-months(1)) %>% 
